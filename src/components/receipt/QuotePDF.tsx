    ...
    <Document title={`Quote - ${project.name}`}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View style={{ flexDirection: 'row', gap: 15, alignItems: 'center' }}>
            ...
            <View>
              ...
              <Text style={{ color: '#666', fontSize: 8 }}>{branding.contact} {branding.email}</Text>
            </View>
          </View>
          ...
          {results.map((res) => (
            <View key={res.id} wrap={false}>
              <View style={styles.tableRow}>
                ...
                <Text style={styles.colQty}>{res.lineItems[0]?.quantity} {res.service.unit}</Text>
                <Text style={styles.colAmount}>{res.total.format()}</Text>
              </View>
              {res.modifiers.map((mod) => (
                <View key={mod.id} style={styles.modifierRow}>
                  <Text style={styles.colDesc}>   + {mod.label}: {mod.optionLabel}</Text>
        ...
        <View style={styles.warningPanel}>
          ...
          <Text style={styles.warningText}>• Fees are based on standard GEPI rate schedules and current legislation.</Text>
          <Text style={styles.warningText}>• Final project costs may vary based on actual field conditions and government verification fees.</Text>
        </View>

        {/* Footer */}
        <View style={{ position: 'absolute', bottom: 30, left: 40, right: 40, borderTop: 1, borderTopColor: '#eee', paddingTop: 10 }}>
          <Text style={{ textAlign: 'center', color: '#999', fontSize: 7 }}>
            {branding.website ? `${branding.name} — ${branding.website}` : `Generated by Quotable — Local-first Schema-driven Engine.`}
          </Text>
        </View>
      </Page>
    </Document>
